...
void ThreadPool::start(int initThreadSize)
{
	...
	for (int i = 0; i < initThreadSize_; i++)
	{
		...
		int threadId = ptr->getId();
		threads_.emplace(threadId, std::move(ptr));
		// threads_.emplace_back(std::move(ptr));a
	}
...
void ThreadPool::threadFunc(int threadid)  // 线程函数返回，相应的线程也就结束了
{
 ...
 // cached模式下，有可能已经创建了很多的线程，但是空闲时间超过60s，应该把多余的线程
			...
			// 锁 + 双重判断
			// 以前不是死循环 是以线程是否在运行作为判断条件
			//死循环 不断看有没有任务 任务没有执行完是不会去判断线程状态的
			//如果有任务 就一直往下去执行任务 等到吧任务消费完了 再去判断状态并remove
			while (taskQue_.size() == 0)
			{
				...
				{
					//替换notEmpty_.wait_for(lock, [&]()->bool {return taskQue_.size() > 0; });
					// 等待notEmpty条件
					notEmpty_.wait(lock);
				}
...
/////////////////  Task方法实现
Task::Task()
	: result_(nullptr)//给Task增加一个接受result的成员变量
{}

void Task::exec()//直接封装了一个run 对返回的Any setValue
{
	if (result_ != nullptr)
	{
...
void Result::setVal(Any any)  // 谁调用的呢？ 把任务的返回值setVal方法给到Result
{
	// 存储task的返回值
